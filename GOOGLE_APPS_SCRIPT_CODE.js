...
function onFormSubmit(e) {
  try {
    ...
    const teamName = rowValues[headers.indexOf("TEAM NAME")] || "Your Team";
    const leadName = rowValues[headers.indexOf("LEAD NAME")] || "Participant";
    const domain = rowValues[headers.indexOf("DOMAINS")] || "General";
    const email = rowValues[headers.indexOf("Email address")];

    if (email) {
      sendBrevoEmail(email, leadName, memberId, teamName, domain);
      Logger.log(`✅ Email sent to: ${email} for Team ${memberId}`);
    } else {
...
function sendBrevoEmail(toEmail, name, teamId, teamName, domain) {
      ...
      <div style="max-width: 650px; margin: 0 auto; background-color: #1a1a1a; padding: 40px; border-radius: 20px; border: 2px solid #D4AF37; box-shadow: 0 0 30px rgba(212, 175, 55, 0.3);">
        ...
        <div style="background: linear-gradient(135deg, #2a0a38, #1a1a1a); padding: 25px; border-radius: 15px; border-left: 4px solid #D4AF37; margin-bottom: 30px;">
          ...
          <p style="margin: 0; font-size: 16px; line-height: 1.6;">
            ...
            Your team has been <strong style="color: #00ffbf;">successfully registered</strong> for TechathonX 2K26! 
  ...
  try {
    ...
    if (code === 201) {
      ...
      Logger.log("⚠️ Email failed. Code: " + code + " | Response: " + response.getContentText());
    }
  } catch (e) {
...
function testBrevoConnection() {
  ...
  sendBrevoEmail(SENDER_EMAIL, "Test Admin", "TX-TEST-001", "Debug Team", "Testing");
